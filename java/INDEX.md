...
Work through the tiers in order — each builds on the previous one. Cross-references to the [Go learning path](../golang/INDEX.md) for a backend-language comparison point (concurrency model, error handling, package system), the [Networking learning path](../networking/INDEX.md) for protocol and infrastructure topics, the [Kubernetes learning path](../kubernetes/INDEX.md) for platform-level K8s understanding, the [Low-Level Design learning path](../low-level-design/INDEX.md) for class-level OOD, GoF design patterns, and OOD interview problems, the [System Design learning path](../system-design/INDEX.md) for HLD, the [Operating Systems learning path](../operating-systems/INDEX.md) for the Linux primitives Java sits on, the [Observability learning path](../observability/INDEX.md) for metrics/logs/traces and SLO design, the [Performance Engineering learning path](../performance/INDEX.md) for latency/throughput/capacity reasoning, the [Security learning path](../security/INDEX.md) for OWASP, OAuth/OIDC, JWT, TLS, and secret management, the [Search learning path](../search/INDEX.md), the [Linux learning path](../linux/INDEX.md), the [Git Internals learning path](../git-internals/INDEX.md), and the [Web Scraping learning path](../web-scraping/INDEX.md) (recon, scraping mechanics, browser-side reverse-engineering — the offensive/data-extraction side of the network and security stack).
...
**Markers:** **★** = core must-learn (everyday Spring Boot backend work, common in interviews and production debugging). **○** = supporting deep-dive (advanced JVM, specialized frameworks, or peripheral topics). Internalize all ★ before going deep on ○.
...
- [Spring Security — Bug Spotting](spring-security-bug-spotting.md) ★ — _(2026-05-03)_
