...
Work through the tiers in order — each builds on the previous one. Cross-references to the [Networking learning path](../networking/INDEX.md) for protocol and infrastructure topics, the [Kubernetes learning path](../kubernetes/INDEX.md) for platform-level K8s understanding, the [Low-Level Design learning path](../low-level-design/INDEX.md) for class-level OOD, GoF design patterns, and OOD interview problems, the [System Design learning path](../system-design/INDEX.md) for HLD, the [Operating Systems learning path](../operating-systems/INDEX.md) for the Linux primitives Java sits on, the [Observability learning path](../observability/INDEX.md) for metrics/logs/traces and SLO design, the [Performance Engineering learning path](../performance/INDEX.md) for latency/throughput/capacity reasoning, and the [Security learning path](../security/INDEX.md) for OWASP, OAuth/OIDC, JWT, TLS, and secret management.
